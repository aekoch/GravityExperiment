...
public class Octree : MonoBehaviour {
    ...
    public int[] cellCountAtDepth;
    public int dataCount;
    public int[] dataCountAtDepth;
    ...
    private void OnRenderObject() {
        if (showOctree) {
            ...
            foreach (Cell cell in root.GetChildrenAndSelf()) {
                if (cell.transforms.Count >= threshold) {
            ...
            if (target && cellByTransform.ContainsKey(target)) {
                foreach (Cell cell in cellByTransform[target].Neighbors()) {
                    if(cell != null) {
                        GL.Color(targetCellNeighborColor);
                        RenderCube cube_ = new RenderCube(cell.center, cell.radius);
                        foreach (Vector3 v in cube_.lineVertices) {
                            GL.Vertex(v);
                        }
                    }
